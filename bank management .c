...

{
    ...
    printf("\nEnter the address:");
    scanf("%s",add.address);
    printf("\nEnter the citizenship number or country:");
...
void view_list()
{
    ...
    while(fscanf(view,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d",&add.acc_no,add.name,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF)
       {
           printf("\n%6d\t %10s\t\t\t%10s\t\t%.0lf\t\t%d",add.acc_no,add.name,add.address,add.phone,add.amt);
           test++;
       }
...
int close()
{
    printf("\n\n\n\nThis C Mini Project is developed by bhal chandra and sathyanarayan");
    exit(0);
    return(0);
	}
